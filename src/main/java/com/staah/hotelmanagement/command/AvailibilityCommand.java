...
public class AvailibilityCommand {
	 ...
	 * @param dateRange
	 * @param roomType
	 * @return this function will return the availibility for the mentioned
	 *         dateRange for a particular roomtype for the particular hotel in the
	 *         input
	 */
	public int queryForAvailibility(String hotelId, String dateRange, String roomType) {
		...
		List<ActualBooking> actualBookingList = bookingObject.getActualBooking(hotelList);// to get booking list from
																							// json
		...
		Long bookedCount = countList.stream().sorted(Comparator.reverseOrder()).findFirst().get();// we have to get the
																									// minimum of the
																									// list to give as a
																									// output.
		long AvailibilityCount = bookingObject.getRoomType(hotelId, roomType, hotelList).getAvailibilityCount();// we
																												// are
																												// getting
																												// the
																												// availibility
																												// count.
		return (int) (AvailibilityCount - bookedCount);// returning the availibility count which can even be negative,
